...
function _initPicker(input){
	input.attr("readOnly","readOnly");
	$("#widgetCalendar").hide();
	var value ;
	if($("#widgetCalendar").css("display") == "none"){
		$("#widgetCalendar").css("top",input.offset().top+input.height()+8).css("left",input.offset().left-100);
		$("#widgetCalendar").show();
		WdatePicker({
			eCont:"_datapanel",
			doubleCalendar:false,
			...
			onpicked:function(dp){
				value= dp.cal.getDateStr();
			}
		});
	}
	if(value == null){
		value = new Date().Format("yyyy-MM-dd").toString();
	}
	$("._datePiSure").die().live('click',function(){
		input.val(value+ " "+$("#_timeSelect").val());
		$("#widgetCalendar").hide();
	});
	$("._datePiCancel").die().live('click',function(){
		$("#widgetCalendar").hide();
	});
	$("._allDay").die().live('click',function(){
		input.val(value+ " 23:59:59");
		$("#widgetCalendar").hide();
	});
}
function _initPickerNav(type){
	var $calen = $('<div id="widgetCalendar" class="_pickerPanel" style="position: absolute;top:0;left:0;display: none;background-color: #F6F6F6;z-index: 9999;border: 1px solid #EDEDE9;width:300px;"></div>');
	var $datapenel = $('<div id="_datapanel" style="float:left;margin: 10px;"></div>');
	var $selectpanel = $('<div id="_selectpanel" style="float:left;"></div>'); 
	var $timeLabel = $('<label style="line-height: 30px;margin: 10px 10px 10px 0px;">时间：</label><br />');  
	var $selectbord = $(' <div class="select_bord"><a  style="width:50px;" class="select_bord_a"></a></div>');     
	var $sele = $('<select id="_timeSelect" tag="sdelay">'+_initSelectOption(type)+'</select>');    
	var $allday = $('<a class="_allDay" style="margin:10px 3px; width:60px;float:right;background: none repeat scroll 0 0 #EF4929;color: #FFFFFF; display: inline-block;font-size: 14px; height: 22px;line-height: 22px;text-align: center;" href="javascript:void(0)">当天</a>');          
	var $cle = $('<div class="clear"></div>');
	var $datebtn = $('<div class="pop_btn" style="text-align: center;margin-top: 5px;margin-bottom: 5px;"><a class="btn mr-25 _datePiSure" href="javascript:void(0)" >确定</a><a class="btn1 _datePiCancel" href="javascript:void(0)">取消</a></div>');
	$selectbord.append($sele);
	$selectbord.after("<br/>");
	$selectpanel.append($timeLabel).append($selectbord).append($allday);
	$calen.append($datapenel).append($selectpanel).append($cle).append($datebtn);
	$('body').append($calen);
	$("#_timeSelect").parent().sSelect();
}
function _initSelectOption(type){
	var str ='';
	if(type == 1){
		str ='<option value ="23:59:59">全部</option>';
	}
	for(var i = 0;i<24;i++){
		str+='<option value="'+i+':00:00">'+i+':00</option>';
	}
	return str;
}

//年份选择
function _initSelectYear(id,value){
	var endYear=new Date().getUTCFullYear();//结束年份，默认为当前年份
	var startYear=2008;//起始年份
